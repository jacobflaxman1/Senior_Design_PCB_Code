...
void debug_raw_accel_data()
{
    uint8_t raw_data[6];
    uint8_t* temp;

    // Attempt to read the raw accelerometer data registers
    temp = read_multiple_icm20948_reg(ub_0, B0_ACCEL_XOUT_H, 6);
    memcpy(raw_data, temp, 6);

    // Print the raw hex values to debug
    printf("Raw Accel Data: ");
    for(int i = 0; i < 6; i++) {
...
bool icm20948_data_ready() {
    uint8_t int_status = read_single_icm20948_reg(ub_0, B0_INT_STATUS);
    return int_status & 0x01; // Assuming the data ready bit is the LSB
}



void icm20948_enable_data_ready_interrupt() {
    // Enable the accelerometer data-ready interrupt
    write_single_icm20948_reg(ub_0, B0_INT_ENABLE, 0x01); // Assuming 0x01 sets RAW_DATA_0_RDY_EN

    // Confirm the accelerometer is enabled and not in sleep mode
    uint8_t pwr_mgmt_1 = read_single_icm20948_reg(ub_0, B0_PWR_MGMT_1);
    uint8_t pwr_mgmt_2 = read_single_icm20948_reg(ub_0, B0_PWR_MGMT_2);

    // Clear the SLEEP bit (bit 6) in PWR_MGMT_1 to wake up the device
    pwr_mgmt_1 &= ~(1 << 6);
    write_single_icm20948_reg(ub_0, B0_PWR_MGMT_1, pwr_mgmt_1);

    // Ensure the accelerometer is not disabled in PWR_MGMT_2
    pwr_mgmt_2 &= ~(1 << 3); // Assuming bit 3 disables the accelerometer
    write_single_icm20948_reg(ub_0, B0_PWR_MGMT_2, pwr_mgmt_2);
...
void icm20948_process(axises* data)
{
	// collect data -> time stamp data
